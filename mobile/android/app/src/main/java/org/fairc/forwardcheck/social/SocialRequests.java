...
final class SocialRequests {
    ...
    static JSONObject body(SocialResearchInput input,String mode,String system) throws JSONException {
        boolean detail=mode.equals("detail"),report=mode.equals("report"),web=detail||mode.equals("news");
        Object userContent=input.text;
        if(detail||report){JSONArray parts=new JSONArray().put(new JSONObject().put("type","text").put("text",input.userText()));
        ...
        JSONObject body=new JSONObject().put("model",detail?SocialPolicy.RESEARCH_MODEL:SocialPolicy.MODEL)
                .put("temperature",0).put("max_tokens",detail?650:report?320:web?180:mode.equals("context")?180:mode.equals("grounded")||mode.equals("screen")?60:80)
